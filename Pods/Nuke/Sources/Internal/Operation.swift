...
final class Operation: Foundation.Operation {
  ...
  init(starter: @escaping Starter) {
    ...
    _isExecuting = UnsafeMutablePointer<Int32>.allocate(capacity: 1)
    _isExecuting.initialize(to: 0)

    _isFinished = UnsafeMutablePointer<Int32>.allocate(capacity: 1)
    _isFinished.initialize(to: 0)

    isFinishCalled = UnsafeMutablePointer<Int32>.allocate(capacity: 1)
    isFinishCalled.initialize(to: 0)
