...
class CosmosLayers {
  ...
  class func createStarLayers(_ rating: Double, settings: CosmosSettings, isRightToLeft: Bool) -> [CALayer] {
    ...
    var ratingRemander = CosmosRating.numberOfFilledStars(
      rating,
    ...
    for _ in 0..<settings.totalStars {
      ...
      let fillLevel = CosmosRating.starFillLevel(
        ratingRemainder: ratingRemander,
   ...
   */
  class func createCompositeStarLayer(
    _ starFillLevel: Double,
