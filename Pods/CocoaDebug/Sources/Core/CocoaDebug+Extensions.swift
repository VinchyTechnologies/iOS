...
extension String {
  func stringToDictionary() -> [String: Any]? {
    stringToData()?.dataToDictionary()
  }
}

extension Dictionary {
  func dictionaryToString() -> String? {
    dictionaryToData()?.dataToString()
  }
}

extension String {
  func jsonStringToPrettyJsonString() -> String? {
    stringToDictionary()?.dictionaryToString()
  }
}
...
extension String {
  func isValidFormString() -> Bool {
    if let _ = formStringToDictionary() {
      return true
    }
    return false
  }
...
extension String {
  func jsonStringToFormString() -> String? {
    stringToDictionary()?.dictionaryToFormString()
  }
}

extension String {
  func formStringToJsonString() -> String? {
    formStringToDictionary()?.dictionaryToString()
  }
}

extension String {
  func formStringToData() -> Data? {
    formStringToDictionary()?.dictionaryToData()
  }
}

extension Data {
  func formDataToDictionary() -> [String: Any]? {
    dataToString()?.formStringToDictionary()
  }
}
...
extension Data {
  func dataToPrettyPrintString() -> String? {
    //1.pretty json
    if let str = dataToDictionary()?.dictionaryToString() {
      return str
    } else {
      //2.protobuf
...
extension UIWindow {
  ...
  open override func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
    ...
    if cocoadebugShakeProperty == true {
      cocoadebugShakeProperty = false
      return
    }
