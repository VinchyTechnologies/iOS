...
// MARK: - NetworkDetailViewController
...
class NetworkDetailViewController: UITableViewController, MFMailComposeViewControllerDelegate {
  ...
  var detailModels = [NetworkDetailModel]()
...
//        NotificationCenter.default.removeObserver(self)
    ...
    if
      let index = httpModels?.firstIndex(where: { model -> Bool in
  ...
  func close(_ sender: UIBarButtonItem) {
    (navigationController as! CocoaDebugNavigationController).exit()
  }

  @IBAction
  func didTapMail(_ sender: UIBarButtonItem) {
...
//        CocoaDebugSettings.shared.responseShakeNetworkDetail = false
    ...
    // create an action
    let firstAction = UIAlertAction(title: "share via email", style: .default) { [weak self] action -> Void in
      if let mailComposeViewController = self?.configureMailComposer() {
        self?.present(mailComposeViewController, animated: true, completion: nil)
      }
    }

    let secondAction = UIAlertAction(title: "copy to clipboard", style: .default) { [weak self] action -> Void in
