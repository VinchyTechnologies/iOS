...
struct NetworkDetailModel {
  ...
  init(title: String? = nil, content: String? = "", url: String? = "", image: UIImage? = nil, httpModel: _HttpModel? = nil) {
    ...
    if title == "REQUEST" {
      requestData = httpModel?.requestData

      guard let content = content, let url = url, let data = requestData, let keys = CocoaDebugSettings.shared.protobufTransferMap?.keys else {return}
      ...
      for key in keys {
        if url.contains(key) {
          //1.
          guard let arr : [String] = CocoaDebugSettings.shared.protobufTransferMap?[key] else {return}
          if arr.count == 2 {
            //2.
      ...
      responseData = httpModel?.responseData

      guard let content = content, let url = url, let data = responseData, let keys = CocoaDebugSettings.shared.protobufTransferMap?.keys else {return}
      if !content.contains("_GPBMessage") {return}
  ...
  var requestSerializer = RequestSerializer.JSON//默认JSON格式
