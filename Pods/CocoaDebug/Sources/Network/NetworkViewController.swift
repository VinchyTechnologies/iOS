...
class NetworkViewController: UIViewController {
  ...
  var models: [_HttpModel]?
  var cacheModels: [_HttpModel]?
  var searchModels: [_HttpModel]?
  ...
  func searchLogic(_ searchText: String = "") {
    ...
    if searchText == "" {
      ...
      for _ in searchModels {
        if
          let index = self.searchModels?.firstIndex(where: { model -> Bool in
            !model.url.absoluteString.lowercased().contains(searchText.lowercased())//忽略大小写
          })
        {
  ...
  func reloadHttp(needScrollToEnd: Bool = false) {
    ...
    models = (_HttpDatasource.shared().httpModels as NSArray as? [_HttpModel])
    cacheModels = models

    searchLogic(CocoaDebugSettings.shared.networkSearchWord ?? "")
  ...
  @IBAction
  func tapTrashButton(_ sender: UIBarButtonItem) {
...
extension NetworkViewController: UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
  {
    ...
    let vc = NetworkDetailViewController.instanceFromStoryBoard()
  ...
  func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
    if editingStyle == .delete {
      ...
      self.models?.remove(at: indexPath.row)
      _ = cacheModels?.firstIndex(of: model).map { self.cacheModels?.remove(at: $0) }
      _ = searchModels?.firstIndex(of: model).map { self.searchModels?.remove(at: $0) }
