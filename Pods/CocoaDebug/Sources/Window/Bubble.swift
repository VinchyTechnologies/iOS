...
//        }
  ...
  static var size: CGSize {CGSize(width: _width, height: _height)}
  ...
  @objc
  func deleteAllLogs_notification() {
    dispatch_main_async_safe { [weak self] in
  ...
  func panDidFire(panner: UIPanGestureRecognizer) {
    ...
    panner.setTranslation(CGPoint.zero, in: superview)
    ...
    if panner.state == .ended || panner.state == .cancelled {
      ...
      var finalX = Double(width/8*4.25)
      var finalY = Double(location.y)

      if location.x > UIScreen.main.bounds.size.width / 2 {
        finalX = Double(UIScreen.main.bounds.size.width) - Double(width/8*4.25)
...
//                numberLabel?.frame = CGRect(x:_width - 10, y:-10, width:20, height:20)
      ...
      if finalY > Double(UIScreen.main.bounds.size.height) - Double(height/8*4.25) {
        finalY = Double(UIScreen.main.bounds.size.height) - Double(frameInset.bottom) - Double(height/8*4.25)
      } else if finalY < Double(height/8*4.25) + Double(frameInset.top) {
        finalY = Double(height/8*4.25) + Double(frameInset.top)
      }
  ...
  fileprivate func initLabelEvent(_ content: String, _ foo: Bool) {
    if content == "🚀" || content == "❌"
    {
      ...
      //step 2
      if foo == true {
        label.frame = CGRect(x: frame.size.width/2 - WH/2, y: frame.size.height/2 - WH/2, width: WH, height: WH)
        addSubview(label)
      } else {
        label.frame = CGRect(x: center.x - WH/2, y: center.y - WH/2, width: WH, height: WH)
