...
open class FadingInfoViewController: UIViewController {
  ...
  public init(headerViewController: HeaderViewController,
  ...
  private func updateHeaderContainerHeight(with contentHeight: CGFloat, animate: Bool) {
    let minimalHeaderContainerHeight = self.view.frame.height - contentViewController.minimalHeight
...
extension FadingInfoViewController: FadingInfoContentViewDelegate {
  public func fadingInfoContentViewDidRequestShowFailure(_ view: FadingInfoContentViewProtocol) {
    headerViewController.showError()
  }
  
  public func fadingInfoContentViewDidUpdateVerticalOffset(_ view: FadingInfoContentViewProtocol) {
  ...
  public func fadingInfoContentView(_ contentView: FadingInfoContentViewProtocol, didRequestAnimateReloadWith view: UIView) {
...
extension FadingInfoViewController: FadingInfoHeaderViewDelegate {
  ...
  public func fadingInfoHeaderViewDidRequestShowEmbeddedInBottomSheet(_ viewController: UIViewController) {
    guard let presentationCenter = presentationCenter,
  ...
  public func fadingInfoHeaderViewDidUpdateVerticalOffset(_ view: FadingInfoHeaderViewProtocol) {
    updateContainerTransition()
    updateContentDecoratorViewPosition()
  }
...
extension FadingInfoViewController: FadingInfoViewDelegate {
  ...
  func fadingInfoView(_ view: FadingInfoView, hitTest point: CGPoint, with event: UIEvent?) -> UIView? {
    return point.y < contentDecoratorView.frame.minY
...
public extension FadingInfoHeaderViewProtocol {
  func showError() { }
}
...
public extension UIViewController {
  ...
  func embedChild(_ viewController: UIViewController, to container: UIView? = nil) {
  ...
  func unembedFromParentIfNeeded() {
...
public extension OldBottomSheetChildProtocol {
...
public extension OldBottomSheetParentProtocol {
...
public class OldBottomSheetContainerViewController: UIViewController, OverlayPresentable {
  ...
  public var bluredView: UIVisualEffectView!
  ...
  private func show() {
    ...
    topController.view.frame = CGRect(x: 0, y: arrowView.frame.origin.y + arrowView.bounds.height, width: topView.bounds.width,
...
extension OldBottomSheetContainerViewController: UIGestureRecognizerDelegate {
  ...
  @objc public func panned(recognizer: UIPanGestureRecognizer) {
    ...
    if recognizer.state == .ended {
      ...
      if state == .open {
        if (scrollView!.contentOffset.y + topView.bounds.height) > scrollView!.contentSize.height {
          ...
          let y = min(self.scrollView!.contentOffset.y - velocity.y / 5, self.scrollView!.contentSize.height - self.scrollView!.bounds.height)
          self.scrollView!.setContentOffset(CGPoint(x: scrollView!.contentOffset.x, y: y), animated: true)
        }
      }
    }
  }
...
open class BaseSlidingViewController: UIViewController, OverlayPresentable {
  ...
  convenience public init?(topViewController: ChildViewControllerDelegate, keyboardAffectsTopPadding keyboardAffects: Bool) {
  ...
  public func present(topViewController: ChildViewControllerDelegate,
                      topPadding: CGFloat) {
    ...
    UIView.animate(withDuration: 0.35, animations: {
      ...
      self.topView!.frame = self.topView!.frame.offsetBy(dx: .zero,
                                                         dy: height)
