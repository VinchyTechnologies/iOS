...
extension UIDevice {
  public static var type: Model {
    ...
    if let model = modelMap[String(validatingUTF8: modelCode!)!] { // swiftlint:disable:this force_unwrapping
      if model == .simulator {
        if let simModelCode = ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] {
          if let simModel = modelMap[String(validatingUTF8: simModelCode)!] { // swiftlint:disable:this force_unwrapping
            return simModel
          }
        }
      }
