    ...
    Deferred {
      Future { promise in
        Partners.shared.getNearestPartners(
          numberOfPartners: 5, latitude: userLocation.latitude, longitude: userLocation.longitude, radius: radius, accountID: UserDefaultsConfig.accountID, completion: { result in
            promise(result)
          })
      }
    }
