...
final class ProtocolInterceptor: NSObject {
  ...
  class func forProtocols(protocols: [Protocol]) -> ProtocolInterceptor {
    ...
    let theConcreteClass = concreteClassWithProtocols(
      protocols: protocols,
      concatenatedName: concatenatedName,
      salt: nil)

    let protocolInterceptor = theConcreteClass.init()
      as! ProtocolInterceptor // swiftlint:disable:this force_cast
  ...
  override func forwardingTarget(for aSelector: Selector) -> Any? {
    if
      middleMan?.responds(to: aSelector) == true,
    ...
    -> NSObject.Type
  {
    ...
    if let theClass = NSClassFromString(className) {
      ...
      let subclass = objc_allocateClassPair(
        ProtocolInterceptor.self,
        className,
        0)
        as! NSObject.Type // swiftlint:disable:this force_cast
