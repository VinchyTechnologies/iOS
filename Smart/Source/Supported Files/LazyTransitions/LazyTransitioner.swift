...
public class LazyTransitioner: NSObject {
  ...
  public init(
    lazyScreen: UIViewController,
    transition: TransitionType,
    animator: TransitionAnimatorType? = nil,
    interactor: TransitionInteractor? = nil,
    presentation: Presentation? = nil)
  {
    internalInteractor = interactor ?? TransitionInteractor.default()
    internalAnimator = animator ?? transition.animator
    weak var weakLazyScreen = lazyScreen
    triggerTransitionAction = transition.transitionTrigger(weakLazyScreen)
    transitionCombinator = TransitionCombinator(defaultAnimator: internalAnimator)
    self.presentation = presentation
    transitionCombinator.allowedOrientations = animator?.allowedOrientations ?? transition.allowedOrientations
  ...
  public init(
    animator: TransitionAnimatorType = DismissAnimator(orientation: .topToBottom),
    interactor: TransitionInteractor = TransitionInteractor.default())
  {
    internalAnimator = animator
    internalInteractor = interactor
    transitionCombinator = TransitionCombinator(defaultAnimator: animator)
    presentation = nil
    transitionType = nil
  ...
  deinit {
    scrollViewInterceptors.forEach { scrollView, _/*interceptor*/ in
  ...
  public var onCompleteTransition: () -> Void = {}

  public var triggerTransitionAction: (LazyTransitioner) -> Void = { _ in }

  public var animator: TransitionAnimatorType {
    transitionCombinator.animator
  }

  public var interactor: TransitionInteractor? {
    transitionCombinator.interactor
  }
    ...
    context: UnsafeMutableRawPointer?)
  {
    ...
    if keyPath == parentViewControllerKey {
      ...
      if
        let navVC = oldParent as? UINavigationController,
        newParent is NSNull
      {
  ...
  fileprivate func createTransitioners(for scrollView: UIScrollView) -> [TransitionerType] {
    ...
    let scrollViewTransitioner = InteractiveTransitioner(
      with: scrollViewGestureHandler,
      with: internalAnimator,
      with: internalInteractor)
...
extension LazyTransitioner: UIViewControllerTransitioningDelegate {
  ...
  {
    presentation?.presentationControllerInit(presented, presenting, source)
  }
}
